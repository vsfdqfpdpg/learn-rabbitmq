...
var FirstConsumer = &cobra.Command{
	Use:   "pub_sub_first_consumer",
	Short: "First consumer to consume pub sub queue",
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			for message := range messages {
				log.Printf("Pub Sub First Consumer: %s", message.Body)
				message.Ack(false)
			}
		}()
