...
var FirstCosumer = &cobra.Command{
	Use:   "work_queue_first_consumer",
	Short: "First consumer for work queue",
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			for message := range messages {
				log.Printf("Work queue first consumer: %s\n", message.Body)
				time.Sleep(time.Second * time.Duration(utils.RandInt(5, 3)))
				message.Ack(false)
			}
		}()
